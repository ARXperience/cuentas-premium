...
import type { ParsedAccountMessage, ParsedDeliveryAccount } from './types.js';
...
function splitServiceBlocks(text: string) {
  ...
  for (const line of lines) {
    ...
    if ((looksLikeTitle || startsNewAccount) && current.length) {
      ...
      if (!serviceKey && currentService && !serviceKeyFromText(line) && current.length === 0) current.push(currentService);
      current.push(line);
    }
    if (serviceKey) currentService = line;
  }
  if (current.length) blocks.push(current.join('\n'));
  return blocks;
}

function parseLooseLine(text: string): ParsedDeliveryAccount | null {
...
export function parseAccountMessage(message: string): ParsedAccountMessage {
  ...
  const accounts: ParsedDeliveryAccount[] = [];
  let lastService = '';

  for (const blockText of blocks) {
    const service = serviceKeyFromText(blockText) || lastService || 'servicio';
    if (service !== 'servicio') lastService = service;
  ...
  let confidence = 35;
  if (orderHint) confidence += 25;
  if (accounts.length > 0) confidence += 15;
  if (completeAccounts.length === accounts.length && accounts.length > 0) confidence += 25;
  if (accounts.some((account) => account.service === 'servicio')) confidence -= 10;
