...
import { buildCuentaCobroPdf, CUENTA_COBRO_KEYS, CUENTA_COBRO_DEFAULTS } from './services/cuentaCobro.js';
...
function assertProductionEnv() {
    ...
    if ((process.env.APP_ENCRYPTION_KEY || '').length < 32)
        throw new Error('APP_ENCRYPTION_KEY debe tener al menos 32 caracteres y ser estable.');
}
assertProductionEnv();
...
function loginAttemptKey(req, accessCode) {
    const codeKey = crypto.createHash('sha256').update(accessCode).digest('hex').slice(0, 16);
    return `${ipKeyGenerator(req.ip || 'unknown')}:${codeKey}`;
}
...
const orderStatusValues = ['admin_payment_pending', 'provider_delivery_pending', 'wallet_pending', 'payout_processing', 'pending_payment', 'paid', 'pending', 'processing', 'delivered', 'payout_failed', 'payment_failed', 'cancelled'];
...
const formatDateTimeCO = (value) => new Intl.DateTimeFormat('es-CO', {
    dateStyle: 'short',
    timeStyle: 'short',
...
function keyBytes(secret) {
    if (!secret)
        throw new Error('APP_ENCRYPTION_KEY es requerido.');
...
function safeUser(user, includePhone = true) {
    if (!user)
        return user || null;
    return {
        id: user.id,
...
function serializeOrder(order, viewer) {
    const canSeeSecrets = viewer?.role === 'admin' ||
    ...
    const serializedDeliveries = (order.deliveries || []).map((delivery) => ({
        ...delivery,
    ...
    const serializedItems = (order.items || []).map((item) => ({
        ...
        unit_price: isProviderViewer && !isAdminViewer ? (item.product?.provider_cost || Math.round((order.provider_total || order.total) / Math.max(1, (order.items || []).reduce((sum, orderItem) => sum + orderItem.quantity, 0)))) : item.unit_price,
        subtotal: isProviderViewer && !isAdminViewer ? (item.product?.provider_cost || Math.round((order.provider_total || order.total) / Math.max(1, (order.items || []).reduce((sum, orderItem) => sum + orderItem.quantity, 0)))) * item.quantity : item.subtotal,
        delivered_accounts: serializedDeliveries.filter((delivery) => delivery.order_item_id === item.id)
    }));
    return {
        ...order,
        ...
        providerPayouts: viewer?.role === 'admin' || viewer?.role === 'provider'
            ? (order.providerPayouts || []).map((payout) => ({
                id: payout.id,
...
function serializeProduct(product, viewer) {
...
async function createNotification(user_id, order_id, type, title, message) {
    return prisma.notification.create({
        data: { user_id, order_id, type, title, message }
    });
}
async function createRoleNotifications(role, order_id, type, title, message) {
...
function billingScheduleForPeriod(period) {
...
async function notifyAdminInvoiceCreated(invoice) {
    if (invoice.admin_notified_at)
        return;
    await createRoleNotifications('admin', null, 'invoice.generated', 'Factura mensual creada', `Factura ${invoice.invoice_number} creada para ${invoice.user.name}. Total: ${money(invoice.total_amount)}.`);
...
async function generateClientInvoiceInner(user, period = periodForBogotaDate(), actorId, notifyAdmin = true, range = invoiceRange(period)) {
    ...
    const invoice = await prisma.$transaction(async (tx) => {
        ...
        const totalAmount = saved.lines.reduce((sum, line) => sum + line.total, 0);
    ...
    await addMovement(existing ? 'invoice.synced' : 'invoice.generated', `${existing ? 'Factura actualizada' : 'Factura generada'} para ${user.name}, periodo ${period}, total ${money(invoice.total_amount)}.`, actorId, undefined);
    if (notifyAdmin && !existing)
        await notifyAdminInvoiceCreated(invoice);
    return invoice;
}
...
function normalizeWhatsAppPhoneForCompare(value) {
...
function isInboundAllowed(from) {
...
function accountHasMinimumData(account) {
...
async function findOrderForParsedMessage(parsed) {
    const pendingWhere = { deleted_at: null, status: { notIn: ['delivered', 'cancelled', 'payment_failed', 'payout_failed'] } };
    if (parsed.orderHint) {
    ...
    const serviceKeys = parsed.accounts.map((account) => serviceKeyFromText(account.service)).filter(Boolean);
    ...
    const matches = candidates.filter((order) => {
        ...
        return serviceKeys.every((key) => orderKeys.includes(key));
    });
    return { order: matches.length === 1 ? matches[0] : null, ambiguous: matches.length > 1 };
}
async function deliveryActorForOrder(order) {
    ...
    if (!admin)
        throw new Error('No hay usuario proveedor o admin para registrar la entrega.');
    return admin.id;
}
async function refreshOrderDeliveryStatus(orderId, actorId) {
...
async function createDeliveriesFromParsed(order, parsed, actorId) {
    ...
    if (created > 0) {
        await createNotification(order.user_id, order.id, 'account.delivered', 'Tu cuenta ya esta disponible', `Tu cuenta ya esta disponible. Orden ${order.order_number}.`);
...
async function createDeliveriesFromAdminItems(order, items, actorId) {
    let created = 0;
    const orderItemById = new Map((order.items || []).map((item) => [item.id, item]));
    for (const item of items) {
        if (!item.matchedOrderItemId || !item.matchedProductId)
            throw new Error('Cada cuenta debe tener producto y item de pedido relacionado.');
        const orderItem = orderItemById.get(item.matchedOrderItemId);
        if (!orderItem || orderItem.product_id !== item.matchedProductId)
            throw new Error('El producto no pertenece al pedido seleccionado.');
        if (!((item.delivered_email || item.delivered_user) && item.delivered_password))
            throw new Error(`Faltan usuario/correo o contrasena para ${item.serviceName}.`);
...
async function notifyAdminInboundResult(message, orderId) {
...
async function createDeliveryDraft(inboundId, parsed, orderId, status = 'needs_review') {
...
async function processInboundDeliveryMessage(payload) {
    ...
    if (!isInboundAllowed(payload.from))
        return;
    try {
...
async function getAnyProviderPaymentConfig(providerId) {
...
async function notifyAdminPaymentByEmail(order, payout, channel = 'email') {
    try {
...
function startWhatsAppBridgeAlertMonitor() {
    ...
    setInterval(() => {
        ...
        void (async () => {
            ...
            if (elapsed < whatsappDisconnectAlertGraceMs() || (alreadySent === 'true' && cooldownElapsed < whatsappDisconnectAlertCooldownMs()))
                return;
...
async function handleWhatsAppOutboxFinalFailure(item) {
    ...
    if (!order || !payout)
        return 'skipped';
    if (order.admin_notification_channel === 'email')
        return 'email';
    if (order.admin_notification_channel === 'whatsapp')
        return 'whatsapp';
...
app.post('/api/auth/login', async (req, res, next) => {
    try {
        ...
        void addMovement('user.login', `Inicio de sesion: ${user.email}`, user.id).catch((error) => {
...
app.patch('/api/admin/products/:id', requireAuth, requireRole('admin'), async (req, res, next) => {
    try {
        ...
        const type = previous.active !== product.active
            ? product.active
                ? 'product.enabled'
                : 'product.disabled'
            : 'product.updated';
...
app.post('/api/orders', sensitiveLimiter, requireAuth, requireRole('client'), async (req, res, next) => {
    try {
        ...
        if (products.length !== productIds.length)
            return res.status(400).json({ message: 'Hay productos invalidos o inactivos.' });
        const provider = await prisma.user.findFirst({ where: { role: 'provider' }, orderBy: { created_at: 'asc' } });
        ...
        await createRoleNotifications('admin', result.order.id, 'order.created', 'Nuevo pedido pendiente', `Orden ${result.order.order_number}: venta ${money(saleTotal)}, proveedor ${money(providerTotal)}, utilidad ${money(profitTotal)}.`);
    ...
    try {
        const where = req.user.role === 'admin'
            ...
            : req.user.role === 'provider'
                ? { deleted_at: null, status: { notIn: ['cancelled', 'payment_failed', 'payout_failed'] }, OR: [{ provider_id: req.user.id }, { provider_id: null }] }
...
app.patch('/api/orders/:id/status', sensitiveLimiter, requireAuth, requireRole('provider', 'admin'), async (req, res, next) => {
    try {
        ...
        if (order.deleted_at)
            return res.status(400).json({ message: 'Este pedido esta en la papelera.' });
...
app.patch('/api/orders/:id/cancel', sensitiveLimiter, requireAuth, requireRole('client'), async (req, res, next) => {
    try {
        ...
        if (order.status === 'cancelled')
            return res.status(400).json({ message: 'Este pedido ya esta cancelado.' });
...
app.post('/api/orders/:id/deliveries', sensitiveLimiter, requireAuth, requireRole('provider', 'admin'), async (req, res, next) => {
    try {
        ...
        if (!item)
            return res.status(400).json({ message: 'El item no pertenece al pedido.' });
        const existingDeliveries = await prisma.deliveredAccount.count({ where: { order_id: order.id, order_item_id: item.id } });
        if (existingDeliveries >= item.quantity)
            return res.status(400).json({ message: 'Este producto ya fue entregado completamente.' });
        await prisma.deliveredAccount.create({
            data: {
                order_id: order.id,
                order_item_id: item.id,
...
const accountReportReasons = ['defective', 'missing_code', 'expired', 'screen_changed', 'credentials_invalid', 'profile_missing', 'other'];
const accountReportStatuses = ['open', 'reviewing', 'resolved', 'rejected'];
...
app.post('/api/account-reports', sensitiveLimiter, requireAuth, requireRole('client'), async (req, res, next) => {
    try {
        ...
        if (activeReport)
            return res.status(409).json({ message: 'Esta cuenta ya tiene un reporte abierto o en revision.' });
        ...
        await createRoleNotifications('admin', deliveredAccount.order_id, 'account.reported', 'Nueva cuenta reportada', `${report.user.name} reporto una cuenta de ${deliveredAccount.orderItem.product_name} en la orden ${deliveredAccount.order.order_number}.`);
...
app.patch('/api/admin/account-reports/:id', sensitiveLimiter, requireAuth, requireRole('admin'), async (req, res, next) => {
    try {
        ...
        await createNotification(report.user_id, report.order_id, 'account.report_updated', 'Actualizacion de tu reporte', `El reporte de ${report.deliveredAccount.orderItem.product_name} en la orden ${report.order.order_number} ahora esta ${statusLabel}.${input.admin_notes ? ` ${input.admin_notes}` : ''}`);
...
app.patch('/api/notifications/:id/read', sensitiveLimiter, requireAuth, requireRole('client', 'admin'), async (req, res, next) => {
    try {
        ...
        if (notification.user_id !== req.user.id)
            return res.status(403).json({ message: 'No puedes modificar esta notificacion.' });
...
app.get('/api/admin/dashboard', requireAuth, requireRole('admin'), async (req, res, next) => {
    try {
        ...
        const [orders, usersCount, activeProducts, deliveredAccounts, recentOrders, movements, notificationSent, notificationPending, notificationFailed, lastProviderNotified, payments, latestPayments, receiptSent, receiptFailed, clients, providerPayouts, latestProviderPayouts, deliveryDraftsPending, whatsappInboundReceived, whatsappInboundAutoDelivered, whatsappInboundDrafts, whatsappInboundFailed] = await Promise.all([
        ...
        const receiptStatusByOrder = new Map(receiptMovements.map((movement) => [
            movement.order_id,
...
async function loadCuentaCobroConfig() {
    const entries = await Promise.all(CUENTA_COBRO_KEYS.map(async (key) => [key, await getSettingValue(key, CUENTA_COBRO_DEFAULTS[key])]));
...
app.patch('/api/admin/cuenta-cobro/config', sensitiveLimiter, requireAuth, requireRole('admin'), async (req, res, next) => {
    try {
        ...
        for (const key of CUENTA_COBRO_KEYS) {
            ...
            if (value !== undefined)
                await upsertSetting(key, value.trim(), false);
        }
...
app.get('/api/admin/orders/pending-delivery', requireAuth, requireRole('admin'), async (req, res, next) => {
    try {
        const orders = await prisma.order.findMany({
            where: {
                deleted_at: null,
                status: { notIn: ['delivered', 'cancelled', 'payment_failed', 'payout_failed'] },
                user: {
                    role: 'client',
...
app.patch('/api/admin/orders/:id', sensitiveLimiter, requireAuth, requireRole('admin'), async (req, res, next) => {
    try {
        ...
        const order = await prisma.order.findUniqueOrThrow({ where: { id: paramId(req.params.id) } });
        if (order.deleted_at)
            return res.status(400).json({ message: 'Este pedido esta en la papelera. No se puede editar hasta restaurarlo manualmente en base de datos.' });
...
app.patch('/api/admin/deliveries/:id', sensitiveLimiter, requireAuth, requireRole('admin'), async (req, res, next) => {
    try {
        ...
        if (delivery.order.deleted_at)
            return res.status(400).json({ message: 'Este pedido esta en la papelera.' });
        const data = {};
        if (input.delivered_email !== undefined)
...
app.delete('/api/admin/orders/:id', sensitiveLimiter, requireAuth, requireRole('admin'), async (req, res, next) => {
    try {
        ...
        if (order.deleted_at)
            return res.status(400).json({ message: 'Este pedido ya esta en la papelera.' });
        ...
        await createNotification(order.user_id, order.id, 'order.deleted_to_trash', 'Pedido retirado', `La orden ${order.order_number} fue retirada del panel.`);
...
app.get('/api/admin/logs', requireAuth, requireRole('admin'), async (req, res, next) => {
    try {
        ...
        const orderIds = Array.from(new Set(outbox.map((item) => item.order_id).filter(Boolean)));
        const orderLabels = new Map((await prisma.order.findMany({ where: { id: { in: orderIds } }, select: { id: true, order_number: true } }))
...
app.patch('/api/admin/payouts/:id/mark-receipt-sent', sensitiveLimiter, requireAuth, requireRole('admin'), async (req, res, next) => {
    try {
        ...
        if (payout.order.deleted_at)
            return res.status(400).json({ message: 'Este pedido esta en la papelera.' });
        if (payout.status !== 'pending_admin_payment')
            return res.status(400).json({ message: 'Este payout no esta pendiente de pago admin.' });
        const updated = await prisma.$transaction(async (tx) => {
...
app.patch('/api/admin/payouts/:id/cancel', sensitiveLimiter, requireAuth, requireRole('admin'), async (req, res, next) => {
    try {
        ...
        await createNotification(updated.order.user_id, updated.order.id, 'order.cancelled', 'Pedido cancelado', `La orden ${updated.order.order_number} fue cancelada.`);
...
app.post('/api/admin/whatsapp/test', sensitiveLimiter, requireAuth, requireRole('admin'), async (req, res, next) => {
    try {
        const adminNumber = await getAdminNotificationPhone();
        if (!adminNumber)
            return res.status(400).json({ message: 'Configura primero el numero WhatsApp del admin.' });
        await queueWhatsAppNotification(prisma, {
            recipient: adminNumber,
...
app.post('/api/admin/whatsapp/inbound/simulate', sensitiveLimiter, requireAuth, requireRole('admin'), async (req, res, next) => {
    try {
        if (process.env.NODE_ENV === 'production')
            return res.status(403).json({ message: 'La simulacion de WhatsApp entrante solo esta disponible fuera de produccion.' });
...
app.post('/api/admin/delivery-parser/preview', sensitiveLimiter, requireAuth, requireRole('admin'), async (req, res, next) => {
    try {
        ...
        if (!orderWhere)
            return res.status(400).json({ message: 'Selecciona una orden pendiente antes de interpretar el mensaje.' });
        const order = await prisma.order.findUniqueOrThrow({
            where: orderWhere,
            include: { user: true, provider: true, items: { include: { product: true } }, deliveries: { include: { product: true } }, payments: true, providerPayouts: true }
        });
        if (!isPendingDeliveryStatus(order.status))
            return res.status(400).json({ message: 'Esta orden ya no esta pendiente de entrega.' });
        if (!isServimilUser(order.user))
            return res.status(403).json({ message: 'Solo se pueden procesar ordenes del cliente Servimil.' });
...
app.post('/api/admin/delivery-parser/approve', sensitiveLimiter, requireAuth, requireRole('admin'), async (req, res, next) => {
    try {
        ...
        const orderWhere = input.orderId ? { id: input.orderId } : normalizedOrderNumber ? { order_number: normalizedOrderNumber } : null;
        if (!orderWhere)
            return res.status(400).json({ message: 'No se puede aprobar sin orden seleccionada.' });
        const order = await prisma.order.findUniqueOrThrow({
            where: orderWhere,
            include: { user: true, provider: true, items: { include: { product: true } }, deliveries: { include: { product: true } }, payments: true, providerPayouts: true }
        });
    ...
    try {
        ...
        const order = input.orderId
            ...
            : normalizedOrderNumber
                ...
                : null;
        if (!order)
            return res.status(400).json({ message: 'Selecciona una orden pendiente antes de guardar borrador.' });
        if (!isPendingDeliveryStatus(order.status))
            return res.status(400).json({ message: 'Esta orden ya no esta pendiente de entrega.' });
        const orderUser = await prisma.user.findUnique({ where: { id: order.user_id } });
        if (!orderUser || !isServimilUser(orderUser))
            return res.status(403).json({ message: 'Solo se pueden guardar borradores de Servimil.' });
        const draft = await prisma.deliveryDraft.create({
            data: {
                order_id: order.id,
...
app.get('/api/admin/delivery-drafts', requireAuth, requireRole('admin'), async (_req, res, next) => {
    try {
        ...
        const inboundIds = drafts.map((draft) => draft.inbound_message_id).filter(Boolean);
        const orderIds = drafts.map((draft) => draft.order_id).filter(Boolean);
...
app.patch('/api/admin/delivery-drafts/:id/approve', sensitiveLimiter, requireAuth, requireRole('admin'), async (req, res, next) => {
    try {
        ...
        if (!['needs_review', 'draft_created'].includes(draft.status))
            return res.status(400).json({ message: 'Este borrador ya fue procesado.' });
        const parsed = draft.parsed_data;
        const orderRef = input.order_id || draft.order_id;
        if (!orderRef)
            return res.status(400).json({ message: 'Selecciona un pedido para aprobar este borrador.' });
        const order = await prisma.order.findUniqueOrThrow({
            where: orderRef.startsWith('ORD-') || orderRef.startsWith('CDD-') ? { order_number: orderRef } : { id: orderRef },
            include: { user: true, provider: true, items: { include: { product: true } }, deliveries: true, payments: true, providerPayouts: true }
        });
        const actorId = await deliveryActorForOrder(order);
        const parserItems = parsed.items;
        const { created } = parserItems?.length
            ? await createDeliveriesFromAdminItems(order, parserItems, actorId)
            : await createDeliveriesFromParsed(order, { ...parsed, accounts: input.accounts?.length ? input.accounts : parsed.accounts }, actorId);
        if (!created)
            return res.status(400).json({ message: 'No se pudo crear ninguna entrega desde este borrador.' });
