...
export type CuentaCobroKey = (typeof CUENTA_COBRO_KEYS)[number];
export type CuentaCobroConfig = Record<CuentaCobroKey, string>;

export const CUENTA_COBRO_DEFAULTS: CuentaCobroConfig = {
...
function seccion(n: number): string {
  ...
  if (dd <= 20) out += UNIDADES[dd];
  else if (dd < 30) out += VEINTI[dd - 20];
  else {
...
export function numeroALetras(value: number): string {
  ...
  const partes: string[] = [];
  if (millones) partes.push(millones === 1 ? 'UN MILLON' : `${seccion(millones)} MILLONES`);
  if (miles) partes.push(miles === 1 ? 'MIL' : `${seccion(miles)} MIL`);
...
export type CuentaCobroOptions = {
  money: (value: number) => string;
  formatDate: (value: Date | string | null | undefined) => string;
  config: CuentaCobroConfig;
};
...
function logo() {
  if (!logoBuf) logoBuf = readFileSync(path.resolve(process.cwd(), 'src/assets/centro-digital-imagotipo.png'));
  return logoBuf;
}
function winAnsi(v: string) {
  return String(v ?? '').replace(/[^\x09\x0A\x0D\x20-\xFF]/g, ' ');
}

export async function buildCuentaCobroPdf(invoice: Invoice, opts: CuentaCobroOptions): Promise<Buffer> {
  ...
  const wrap = (s: string, size: number, maxW: number, f: PDFFont = font) => {
    ...
    for (const w of words) {
      ...
      if (f.widthOfTextAtSize(nx, size) > maxW && cur) { out.push(cur); cur = w; } else cur = nx;
    }
    if (cur) out.push(cur);
  ...
  if (config.cc_acreedor_id) text(`NIT/CC ${config.cc_acreedor_id}`, M + d.width + 10, y - 26, 8.5, font, C.muted);
  ...
  const body = (label: string, value: string) => {
    ...
    y -= lh;
    for (let i = 1; i < first.length; i++) { text(first[i], M, y, bodySize, font); y -= lh; }
  };
  ...
  if (config.cc_signer_id) text(`CC/NIT ${config.cc_signer_id}`, M, signY - 27, 8.5, font, C.muted);
  text('Firma', M, signY - (config.cc_signer_id ? 42 : 27) - 0, 8, font, C.muted);

  if (config.cc_contact) center(config.cc_contact, M + 24, 8.5, font, C.muted);
