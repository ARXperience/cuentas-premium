...
function scheduleReconnect(statusCode) {
    if (!isEnabled() || statusCode === DisconnectReason.loggedOut)
        return;
...
export async function initializeWhatsAppClient(prisma) {
    if (!isEnabled()) {
        connection = 'disabled';
        return;
    }
    ...
    try {
        ...
        currentSocket.ev.on('messages.upsert', ({ messages, type }) => {
            if (!inboundHandler || process.env.WHATSAPP_INBOUND_ENABLED !== 'true' || type !== 'notify')
                return;
            for (const message of messages) {
                ...
                if (!body || message.key.fromMe)
                    continue;
                if (from.endsWith('@g.us') && process.env.WHATSAPP_PROCESS_GROUPS !== 'true')
                    continue;
                void inboundHandler({
