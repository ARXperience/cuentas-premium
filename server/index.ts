...
function databaseErrorReason(error: unknown) {
...
function dateOnlyToBogotaUtcStart(value: string) {
...
function formatDateOnlyCO(value: Date) {
...
function invoicePeriodKeyFromRange(startDate: string, endDate: string) {
  if (startDate.slice(0, 7) === endDate.slice(0, 7)) return startDate.slice(0, 7);
  return `${startDate}_a_${endDate}`;
}

function invoiceNumberPeriodKey(period: string) {
  return period.replace(/[^0-9]+/g, '').slice(0, 16) || periodForBogotaDate().replace('-', '');
}

function invoiceRange(period: string, startDate?: string | null, endDate?: string | null) {
  ...
  if (startDate || endDate) {
    if (!fromInput || !toInput) throw new Error('Selecciona fechas validas para generar la factura.');
    const endExclusive = new Date(toInput.getTime() + 24 * 60 * 60 * 1000);
    if (endExclusive <= fromInput) throw new Error('La fecha final debe ser posterior o igual a la fecha inicial.');
...
function parseMaybeDateEndOfBogotaDay(value?: string | null) {
  const dateOnly = value ? dateOnlyToBogotaUtcStart(value) : null;
  if (dateOnly) return new Date(dateOnly.getTime() + 24 * 60 * 60 * 1000 - 1);
...
const invoiceGenLocks = new Map<string, Promise<any>>();
async function generateClientInvoice(
  user: any,
  period = periodForBogotaDate(),
  actorId?: string,
  notifyAdmin = true,
  ...
  try {
    ...
    if (invoiceGenLocks.get(key) === run) invoiceGenLocks.delete(key);
  }
}

async function generateClientInvoiceInner(
  user: any,
  period = periodForBogotaDate(),
  actorId?: string,
  notifyAdmin = true,
  range = invoiceRange(period)
) {
  const { dueDate } = range;
...
function remainingItemCapacity(item: any, deliveredCountByItem: Map<string, number>) {
  return Math.max((item.quantity || 1) - (deliveredCountByItem.get(item.id) || 0), 0);
}

function matchAccountToItem(account: ParsedDeliveryAccount, items: any[], deliveredCountByItem = new Map<string, number>()) {
...
async function createDeliveriesFromParsed(order: any, parsed: ParsedAccountMessage, actorId: string) {
  ...
  for (const account of parsed.accounts) {
    ...
    if (deliveredCount >= item.quantity) continue;
...
function providerUnitCostForItem(item: any) {
  return item.product?.provider_cost > 0
    ? item.product.provider_cost
    : Math.round(item.unit_price * 0.55);
}

function buildAdminPaymentPendingMessages(order: any, payout: any) {
  const requestedAccounts = order.items.map((item: any) => {
...
app.post('/api/admin/products', requireAuth, requireRole('admin'), async (req, res, next) => {
  try {
    ...
    await addMovement(
      'product.created',
      `Producto creado: ${product.name}. Venta ${money(product.price)}, proveedor ${money(product.provider_cost)}, utilidad ${money(product.price - product.provider_cost)}.`,
      req.user!.id
    );
...
app.patch('/api/admin/products/:id', requireAuth, requireRole('admin'), async (req, res, next) => {
  try {
    ...
    await addMovement(
      type,
      `Producto actualizado: ${product.name}${changes.length ? `. Cambios: ${changes.join(', ')}.` : '. Sin cambios de precio o estado.'}`,
      req.user!.id
    );
    res.json({ product });
  } catch (error) {
...
async function handleServimilInvoiceGenerate(req: Request, res: Response, next: NextFunction) {
  try {
...
async function handleClientInvoiceUpdate(req: Request, res: Response, next: NextFunction) {
  try {
...
app.get('/api/admin/whatsapp/status', sensitiveLimiter, requireAuth, requireRole('admin'), async (_req, res, next) => {
  try {
    ...
    if (adminEnabled === 'true') {
      void ensureWhatsAppBridgeStarted().catch((error: unknown) => {
...
app.post('/api/admin/delivery-parser/approve', sensitiveLimiter, requireAuth, requireRole('admin'), async (req, res, next) => {
  try {
    ...
    const compatibleItems = (input.items as DeliveryParserItem[]).filter((item) =>
      item.matchedOrderItemId
      && item.matchedProductId
      && (item.delivered_email || item.delivered_user)
      && item.delivered_password
    );
    if (!compatibleItems.length) return res.status(400).json({ message: 'No hay cuentas completas para aprobar. Asigna el producto y completa usuario/correo y contrasena.' });
