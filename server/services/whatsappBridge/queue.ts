...
export async function retryFailedWhatsAppMessages(prisma: PrismaClient) {
...
export async function processWhatsAppOutbox(prisma: PrismaClient, addMovement: AddMovement, onFinalFailure?: WhatsAppOutboxFailureHandler) {
  ...
  for (const item of items) {
    ...
    if (isOrderNotification && fallbackExpired && fallbackRetryReady && onFinalFailure) {
      ...
      if (fallbackResult === 'email' || fallbackResult === 'whatsapp') {
        ...
        if (fallbackResult === 'email') {
          await addMovement(
            'whatsapp.email_fallback',
            `Aviso WhatsApp para pedido ${item.order_id} reemplazado por correo de respaldo.`,
            undefined,
      ...
      if (fallbackResult === 'failed') {
        ...
        if (!runtime.enabled || runtime.connection !== 'connected') continue;
      }
    }
    ...
    try {
      ...
      let fallbackResult: Awaited<ReturnType<NonNullable<typeof onFinalFailure>>> | null = null;
      ...
      if (finalStatus === 'failed' && item.order_id && onFinalFailure) {
        ...
        if (fallbackResult === 'email') finalStatus = 'email_fallback';
        if (fallbackResult === 'whatsapp') finalStatus = 'sent';
      }
