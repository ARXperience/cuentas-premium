...
const invoiceDocOptions = (invoice: any) => ({
  money,
  formatDateTime: (value: Date | string | null | undefined) => (value ? formatDateTimeCO(value) : '-'),
  clientName: invoice.user?.name || 'Servimil'
});

async function sendClientInvoiceById(invoiceId: string, actorId?: string) {
  const invoice = await prisma.clientInvoice.findUniqueOrThrow({
    where: { id: invoiceId },
    include: { user: true, lines: { include: { order: true }, orderBy: { position: 'asc' } } }
  });
  const to = invoice.user?.email;
  if (!to) throw new Error('El cliente de la factura no tiene correo configurado.');
  if (!emailConfigured()) throw new Error('SMTP no configurado. Configura el correo antes de enviar facturas.');
  try {
...
function startInvoiceSendScheduler() {
  ...
  const run = async () => {
    ...
    try {
      ...
      for (const { id } of due) {
        await sendClientInvoiceById(id).catch((error: unknown) => {
